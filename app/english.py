...
BODY = {
    ...
    'SubRip字幕(.srt)を読み込み、字幕1件を1カットとして歌詞欄に変換します': 'Import a SubRip subtitle (.srt) and convert each cue into one cut in the lyrics field',
    'SRTを読み込む': 'Import SRT',
    'SRTを読み込むと、字幕1件（1キュー）をそのタイミングのまま1つの文章として表示します（<code>\\N</code>を含む改行はつなげて表示、単語単位や複数カットへの再分割はしません）': 'Importing an SRT shows each cue as one sentence held for its own timing (line breaks, including <code>\\N</code>, are joined; English cues are never re-split into words or extra cuts)',
...
UI = {
    ...
    'SRTを読み込めませんでした': 'Could not read the SRT file',
    'SRTを読み込みました（': 'Imported the SRT file (', '行）': ' lines)',
