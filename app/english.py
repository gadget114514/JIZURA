...
BODY = {
    ...
    'SubRip字幕(.srt)を読み込み、タイミングと改行を歌詞欄に変換します': 'Import a SubRip subtitle (.srt) and convert its timing and line breaks into the lyrics field',
    'SRTを読み込む': 'Import SRT',
    'SRTを読み込むと、字幕のタイミングと改行（<code>\\N</code>を含む）をそのままカットの切れ目にします': 'Importing an SRT uses its own timing and line breaks (including <code>\\N</code>) as the cut boundaries',
